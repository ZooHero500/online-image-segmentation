...
- **实施状态**: 2026-06-05 已完成 `/remove-background` 本地 AI MVP，采用 Transformers.js + 站内托管 `imgsplit/rmbg-1.4` RMBG 模型，通过 Web Worker 本地推理，优先 WebGPU、失败回退 WASM，并提供透明 PNG/WebP 与白底 JPG 导出。首次运行前清楚提示模型下载原因、大小、进度、缓存状态与隐私说明。
